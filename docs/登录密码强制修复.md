# 登录密码强制修复

修复时间：2025-11-01

---

## 📋 需求

将登录系统修改为：
1. ✅ 必须输入公司邮箱和密码
2. ✅ 首次出现的邮箱 → 自动注册（使用输入的密码）
3. ✅ 邮箱已存在 → 验证密码
4. ✅ 密码错误 → 提示"密码错误，请联系管理员重置密码"
5. ✅ 移除"向后兼容"逻辑（不再支持无密码登录）

---

## 🔧 修改内容

### 1. 后端验证规则

**文件**：`server/utils/validators.js`

**修改前**：
```javascript
export const loginSchema = z.object({
  email: z.string().email(),
  password: z.string().optional(), // 可选，用于向后兼容
});
```

**修改后**：
```javascript
export const loginSchema = z.object({
  email: z.string().email(),
  password: z.string().min(6, '密码至少6位'),
});
```

**变化**：
- ✅ 密码改为必填
- ✅ 密码最少6位
- ❌ 移除"可选"配置

---

### 2. 后端登录逻辑

**文件**：`server/services/authService.js`

**核心修改**：

```javascript
export async function login({ email, password }) {
  try {
    const result = await pool.query(
      'SELECT id, email, username, password_hash FROM users WHERE email = $1',
      [email]
    );
    
    let user;
    
    if (result.rows.length === 0) {
      // 首次出现的邮箱，使用提供的密码注册新账号
      const username = email.split('@')[0] + '_' + Math.random().toString(36).substring(2, 8);
      const passwordHash = await argon2.hash(password);
      
      const insertResult = await pool.query(
        'INSERT INTO users (email, username, password_hash) VALUES ($1, $2, $3) RETURNING id, email, username, created_at',
        [email, username, passwordHash]
      );
      
      user = insertResult.rows[0];
      
      // 初始化背包、资源、职业状态、基础卡牌...
      
      logger.info({ userId: user.id, email }, 'User registered with email and password');
    } else {
      // 用户存在，验证密码
      const dbUser = result.rows[0];
      
      const passwordMatch = await argon2.verify(dbUser.password_hash, password);
      if (!passwordMatch) {
        throw new Error('密码错误，请联系管理员重置密码');
      }
      
      user = {
        id: dbUser.id,
        email: dbUser.email,
        username: dbUser.username,
        created_at: dbUser.created_at,
      };
      
      logger.info({ userId: user.id }, 'User logged in');
    }
    
    const token = generateToken(user.id);
    
    return {
      user: { ... },
      token,
    };
  } catch (err) {
    logger.error({ err, email }, 'Login error');
    throw err;
  }
}
```

**关键变化**：
- ✅ 移除所有"向后兼容"逻辑
- ✅ 新用户：使用输入的密码哈希后存储
- ✅ 老用户：必须验证密码
- ✅ 密码错误：抛出明确错误信息
- ❌ 不再支持无密码登录

---

### 3. 前端登录界面

**文件**：`client/src/components/auth/AuthScreen.jsx`

**主要修改**：

1. **添加密码状态**
```javascript
const [email, setEmail] = useState('');
const [password, setPassword] = useState('');  // 新增
const [error, setError] = useState('');
```

2. **添加密码处理器**
```javascript
const handlePasswordChange = useCallback((event) => {
    setPassword(event.target.value);
}, []);
```

3. **表单验证增强**
```javascript
const validateForm = useCallback(() => {
    if (!email.trim()) {
        return '请输入邮箱地址';
    }
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(email.trim())) {
        return '请输入有效的邮箱地址';
    }
    if (!password) {
        return '请输入密码';
    }
    if (password.length < 6) {
        return '密码至少6位';
    }
    return '';
}, [email, password]);
```

4. **提交时传递密码**
```javascript
await onLogin({
    email: email.trim(),
    password: password,  // 新增
});
```

5. **UI修改**
```jsx
<div className="auth-field">
    <label htmlFor="email">公司邮箱</label>
    <input
        id="email"
        type="email"
        placeholder="请输入您的公司邮箱"
        value={email}
        onChange={handleEmailChange}
        disabled={loading}
    />
</div>
<div className="auth-field">
    <label htmlFor="password">初始密码</label>
    <input
        id="password"
        type="password"
        autoComplete="current-password"
        placeholder="请输入密码（至少6位）"
        value={password}
        onChange={handlePasswordChange}
        disabled={loading}
    />
</div>
```

6. **提示文案更新**
```jsx
<div className="auth-hint">
    首次登录将自动注册账号。忘记密码请联系管理员。
</div>
```

---

### 4. 前端API调用

**文件**：`client/src/services/api.js`

**修改前**：
```javascript
export function loginRequest({ email }) {
    return apiRequest('/api/auth/login', {
        method: 'POST',
        body: { email },
    });
}
```

**修改后**：
```javascript
export function loginRequest({ email, password }) {
    return apiRequest('/api/auth/login', {
        method: 'POST',
        body: { email, password },
    });
}
```

---

## ✅ 功能验证

### 场景1：新用户注册
```bash
# 输入：
邮箱：newuser@company.com
密码：123456

# 预期：
✅ 自动创建账号
✅ 密码哈希后存储
✅ 初始化背包、资源、卡牌
✅ 返回JWT token
✅ 成功进入游戏
```

### 场景2：老用户登录（正确密码）
```bash
# 输入：
邮箱：existinguser@company.com
密码：correct_password

# 预期：
✅ 验证密码成功
✅ 返回JWT token
✅ 成功进入游戏
```

### 场景3：老用户登录（错误密码）
```bash
# 输入：
邮箱：existinguser@company.com
密码：wrong_password

# 预期：
❌ 密码验证失败
❌ 返回错误："密码错误，请联系管理员重置密码"
❌ 无法登录
```

### 场景4：前端验证
```bash
# 测试1：邮箱为空
预期：❌ "请输入邮箱地址"

# 测试2：邮箱格式错误
预期：❌ "请输入有效的邮箱地址"

# 测试3：密码为空
预期：❌ "请输入密码"

# 测试4：密码少于6位
预期：❌ "密码至少6位"

# 测试5：都正确
预期：✅ 提交到后端
```

---

## 🔒 安全性提升

### 之前（有问题）
- ⚠️ 可以不提供密码登录
- ⚠️ 只检查邮箱存在性
- ⚠️ 安全性依赖"开发环境"假设

### 现在（已改进）
- ✅ 必须提供密码
- ✅ 新用户：密码强制设置
- ✅ 老用户：密码强制验证
- ✅ 密码最少6位
- ✅ 使用Argon2哈希存储
- ✅ 密码错误有明确提示
- ✅ 完全移除无密码登录

---

## 📊 修改影响

### 破坏性变更 ⚠️
- ❌ 不再支持无密码登录
- ❌ 旧的API调用（不传password）会失败

### 向后兼容性
- ✅ 已存在的账号：密码已在数据库中（之前注册时设置）
- ✅ 新账号：使用输入的密码创建
- ✅ 数据库结构无需改动

### 迁移注意事项
如果有已存在用户的密码是随机生成的（旧逻辑），需要：
1. 方案A：管理员重置密码
2. 方案B：在数据库中标记需要重置密码的用户
3. 方案C：提供"重置密码"功能

---

## 🎯 后续建议

### 必要功能（推荐添加）
1. **密码重置功能**
```javascript
// server/routes/auth.js
router.post('/reset-password', adminMiddleware, async (req, res) => {
  const { userId, newPassword } = req.body;
  const passwordHash = await argon2.hash(newPassword);
  await pool.query(
    'UPDATE users SET password_hash = $1 WHERE id = $2',
    [passwordHash, userId]
  );
  res.json({ success: true });
});
```

2. **管理员重置密码界面**
在管理后台添加"重置用户密码"按钮

3. **密码强度提示**
前端显示密码强度（弱/中/强）

### 可选优化
1. 密码复杂度要求（大小写、数字、特殊字符）
2. 密码过期策略（3个月强制改密码）
3. 登录失败次数限制（防暴力破解）
4. 两步验证（2FA）

---

## 🧪 测试清单

- [x] 后端验证规则（密码必填，最少6位）
- [x] 新用户注册流程
- [x] 老用户登录（正确密码）
- [x] 老用户登录（错误密码）
- [x] 前端表单验证
- [x] 前端密码输入框
- [x] API参数传递
- [ ] 实际部署测试
- [ ] 已有用户迁移测试

---

## 📝 相关文件

修改的文件：
- `server/utils/validators.js`
- `server/services/authService.js`
- `client/src/components/auth/AuthScreen.jsx`
- `client/src/services/api.js`

生成的文档：
- `docs/登录密码强制修复.md`（本文件）

---

修复完成时间：2025-11-01  
修复者：AI Code Review System




