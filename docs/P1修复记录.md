...
### 修复前问题
- 顶层 `await redis.connect()` 导致Redis未启动时应用无法启动
- 没有重试机制
- 没有降级处理

### 修复内容

**文件**：`server/db/redis.js`
...
1. **AI合成缓存**（aiService.js L143-149）
```javascript
// 修复前
...
2. **图片生成缓存**（imageService.js L63-68）
```javascript
// 修复前
...
- ✅ 缓存失败时记录警告日志
- ✅ 日志包含详细上下文（userId、recipe_hash、imageId等）
- ✅ 正常日志包含缓存状态（cached: true/false）
- ✅ 便于监控和定位Redis问题
...
| 问题 | 状态 | 修复文件 |
|------|------|----------|
| Redis连接容错 | ✅ 已修复 | `server/db/redis.js` |
| AI缓存日志 | ✅ 已修复 | `server/services/aiService.js`<br>`server/services/imageService.js` |
