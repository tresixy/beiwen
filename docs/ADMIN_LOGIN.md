# 管理员登录指南

## 默认管理员账号

系统提供了默认的管理员账号：

```
用户名: aita
密码: aitaita
邮箱: aita@admin.com
```

---

## 登录步骤

### 方式1: 通过Web界面登录

1. 打开游戏网页
2. 在登录界面输入：
   - **用户名/邮箱**: `aita@admin.com` 或 `aita`
   - **密码**: `aitaita`
3. 点击 "登录" 按钮
4. 登录成功后进入大厅
5. 点击右上角 ⚙️ 设置图标
6. 在设置面板中可以看到管理员专属按钮：
   - 🎴 卡牌数据库
   - 📁 玩家存档

### 方式2: 通过API登录（测试用）

```bash
curl -X POST http://localhost:3000/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"aita@admin.com","password":"aitaita"}'
```

**响应示例：**
```json
{
  "user": {
    "id": 1,
    "email": "aita@admin.com",
    "username": "aita"
  },
  "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."
}
```

---

## 创建管理员账号

### 自动创建（首次启动）

如果数据库中没有管理员账号，可以运行创建脚本：

```bash
# 在项目根目录执行
npm run admin:create
```

或者：

```bash
node server/db/create-admin.js
```

**脚本会：**
- ✅ 创建用户名为 `aita` 的管理员账号
- ✅ 设置角色为 `admin`
- ✅ 初始化管理员数据（50格背包，9999资源）
- ✅ 解锁所有卡牌（每张10个）

**输出示例：**
```
✅ 管理员账户创建成功
   用户名: aita
   密码: aitaita
   邮箱: aita@admin.com
```

### 手动创建管理员

如果需要创建额外的管理员账号：

#### 方法1: 通过数据库

```sql
-- 1. 注册普通账号（通过Web界面或API）
-- 2. 在数据库中将角色改为管理员

UPDATE users 
SET role = 'admin' 
WHERE username = '你的用户名';
```

#### 方法2: 修改创建脚本

编辑 `server/db/create-admin.js`:

```javascript
const adminEmail = 'your@email.com';
const adminUsername = 'yourusername';
const adminPassword = 'yourpassword';
```

然后运行：
```bash
node server/db/create-admin.js
```

---

## 验证管理员身份

### 方法1: 查看设置面板

管理员登录后：
- 大厅右上角 ⚙️ 设置
- 能看到 "🎴 卡牌数据库" 和 "📁 玩家存档" 按钮
- 普通用户看不到这些按钮

### 方法2: 查询数据库

```sql
SELECT id, username, email, role 
FROM users 
WHERE role = 'admin';
```

**预期结果：**
```
 id | username |      email       | role  
----+----------+------------------+-------
  1 | aita     | aita@admin.com   | admin
```

### 方法3: 调用管理员API

```bash
# 先登录获取token
TOKEN="your_jwt_token_here"

# 测试管理员权限
curl -X GET http://localhost:3000/api/admin/dashboard \
  -H "Authorization: Bearer $TOKEN"
```

如果返回403错误，说明不是管理员账号。

---

## 管理员功能

### 1. 卡牌数据库管理

**访问路径:** 大厅 → ⚙️ 设置 → 🎴 卡牌数据库

**功能：**
- 查看所有卡牌（系统卡 + 用户生成卡）
- 按时代、类型、来源筛选
- 创建新的基础卡牌
- 编辑卡牌属性
- 删除卡牌（用户生成卡受保护）
- 批量导入卡牌
- 实时统计

**详细文档:** `docs/卡牌管理使用指南.md`

### 2. 玩家存档管理

**访问路径:** 大厅 → ⚙️ 设置 → 📁 玩家存档

**功能：**
- 查看所有玩家列表
- 搜索玩家（用户名/邮箱）
- 查看玩家详细数据
- 删除玩家存档
- 批量删除
- 重置玩家进度

**详细文档:** `PLAYER_ARCHIVES_ADMIN.md`

### 3. 其他管理功能（预留）

**API端点：**
- `GET /api/admin/dashboard` - 统计数据
- `GET /api/admin/users` - 用户列表
- `PUT /api/admin/users/:userId/role` - 修改用户角色
- `POST /api/admin/users/:userId/resources` - 修改用户资源
- `DELETE /api/admin/cleanup/test` - 清理测试数据

---

## 安全注意事项

### 1. 修改默认密码

⚠️ **强烈建议修改默认管理员密码！**

**方法1: 通过数据库**

```javascript
// 使用 argon2 生成新密码哈希
import argon2 from 'argon2';

const newPassword = '你的新密码';
const passwordHash = await argon2.hash(newPassword);

// 在数据库中更新
UPDATE users 
SET password_hash = '生成的哈希值' 
WHERE username = 'aita';
```

**方法2: 创建新管理员后删除默认账号**

```sql
-- 1. 创建新管理员账号（使用上面的方法）
-- 2. 删除默认账号
DELETE FROM users WHERE username = 'aita';
```

### 2. 限制管理员权限

```sql
-- 查看所有管理员
SELECT username, email, created_at 
FROM users 
WHERE role = 'admin';

-- 取消管理员权限
UPDATE users 
SET role = 'user' 
WHERE username = '要降级的用户名';
```

### 3. 管理员账号保护

- ✅ 管理员账号不能通过管理面板删除
- ✅ 批量删除时自动排除管理员
- ✅ 所有管理操作都有日志记录

---

## 故障排查

### 问题1: 管理员账号不存在

**症状:** 无法使用 `aita` 登录

**解决:**
```bash
npm run admin:create
```

### 问题2: 登录后看不到管理员按钮

**检查角色:**
```sql
SELECT role FROM users WHERE username = 'aita';
```

**如果不是 admin，修复:**
```sql
UPDATE users SET role = 'admin' WHERE username = 'aita';
```

### 问题3: API返回403 Forbidden

**可能原因:**
- Token过期
- 用户不是管理员
- 中间件未正确配置

**测试:**
```bash
# 1. 重新登录获取新token
# 2. 检查数据库role字段
# 3. 查看服务器日志
```

### 问题4: 忘记管理员密码

**重置密码:**
```bash
# 删除现有管理员
psql -d minigame -c "DELETE FROM users WHERE username = 'aita';"

# 重新创建
npm run admin:create
```

或者直接在数据库中更新密码哈希（见上面"修改默认密码"）

---

## 测试脚本

项目提供了管理员功能测试脚本：

```bash
# 测试管理员API
./test-admin.sh
```

**脚本内容：**
```bash
#!/bin/bash

# 登录管理员
ADMIN_LOGIN=$(curl -s -X POST http://localhost:3000/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"aita@admin.com","password":"aitaita"}')

echo "管理员登录响应:"
echo $ADMIN_LOGIN

# 提取token
TOKEN=$(echo $ADMIN_LOGIN | grep -o '"token":"[^"]*"' | cut -d'"' -f4)

# 测试管理员Dashboard
echo -e "\n\n测试管理员Dashboard:"
curl -s -X GET http://localhost:3000/api/admin/dashboard \
  -H "Authorization: Bearer $TOKEN" | jq .

# 测试卡牌数据库
echo -e "\n\n测试卡牌数据库:"
curl -s -X GET http://localhost:3000/api/cards-database/ \
  -H "Authorization: Bearer $TOKEN" | jq .
```

---

## 环境变量

管理员功能需要正确配置JWT密钥：

```bash
# .env
JWT_SECRET=your-secret-key-here
JWT_EXPIRES_IN=7d
```

如果JWT_SECRET未设置，系统会使用默认值（不推荐生产环境）。

---

## 总结

### 快速开始

```bash
# 1. 创建管理员账号
npm run admin:create

# 2. 启动服务
npm start

# 3. 访问游戏网页
# 4. 使用 aita / aitaita 登录
# 5. 点击设置查看管理员功能
```

### 默认凭证

```
用户名: aita
密码: aitaita
邮箱: aita@admin.com
```

### 管理员功能入口

```
大厅 → ⚙️ 设置 → 管理员按钮
  ├─ 🎴 卡牌数据库
  └─ 📁 玩家存档
```

### 安全提醒

⚠️ **生产环境必须：**
1. 修改默认管理员密码
2. 设置强JWT密钥
3. 启用HTTPS
4. 配置防火墙规则
5. 定期审计管理员操作日志

